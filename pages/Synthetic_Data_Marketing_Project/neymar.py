...
def load_kpis() -> pd.DataFrame:
    """
    Load the five KPI CSVs; ensure files exist and are non-empty.
    Returns concatenated KPI DataFrame.
    """
    required = ["watch_topic.csv", "purchase_events_topic.csv",
                "streams_topic.csv", "partners_topic.csv", "games_topic.csv"]
    missing = []
    ...
    # Top 10 viewed
    viewed = watch.groupby('country')['length']
...
# Top 8 purchased
purchased = purchase.groupby('product_name')
.size().nlargest(8)
...
streamer = sp.groupby('screen_name')['score']
.sum().nlargest(10)
.reset_index()
...
def update_year(y: int):
    ...
    fig = px.choropleth(grouped, locations='country', locationmode='country names', color='pct_rank',
                        hover_data=['watch_hours', 'pct_rank'], color_continuous_scale='Viridis', range_color=(0, 1))
...
def main():
    ...
    with tabs[0]:
        ...
        if st.button("Sample Kafka (99MB each)"):
            res = {t: sample_topic_to_size(t) for t in
                   ['watch_topic', 'purchase_events_topic', 'streams_topic', 'partners_topic', 'games_topic',
                    'customers_topic']}
            st.json(res)
        ...
        if st.button("Chunk all CSVs (~40MB)"):
            out = {p.stem: [str(x) for x in chunk_df_to_size(load_topic_csv(p.stem), p.stem)] for p in
                   DATA_DIR.glob('*_topic.csv')}
            st.json(out)
        st.markdown("---")
    ...
    with tabs[4]:
        ...
        if 'seg' not in st.session_state: st.session_state.seg = {'coords': pd.DataFrame(), 'df': pd.DataFrame(),
        ...
        if res['df'].empty:
            ...
            if len(res['centers']): fig.add_scatter(x=res['centers'][:, 0], y=res['centers'][:, 1], mode='markers',
